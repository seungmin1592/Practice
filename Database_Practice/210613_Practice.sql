...
select sysdate, trunc(sysdate - to_date(20210101,'YYYY-MM-DD'))
from dual;
...
-- 23. 모든 사원의 급여 
--     최고액, 최저액, 총액 및 평균 급여를 출력하시오. 
--     평균에 대해서는 정수로 반올림하시오.
select max(sal) as "최고액", min(sal) as "최저액", sum(sal) as "총액", round(avg(sal)) as "평균 급여"
from emp
;
...
select job, max(sal) as "최고액", min(sal) as "최저액", sum(sal) as "총액", round(avg(sal)) as "평균 급여"
from emp
group by job
;
